...
**Why BLAS for matmul?** Truthfully, I was lazy to implement the matrix multiplication from scratch and optimize it. Don't get me wrong I have tried to be innovative about it and implement a system where based on an estimated computation cost I would determine if i need to process the multiplication using CPU or a GPU ( a very cool concept ), but the amount of bugs pissed me the fuck of so I gave up. Therefore I used BLAS. A more professional answer would be - `cblas_dgemm` is highly optimised for the target architecture and uses cache-blocking and SIMD internally. Writing a competitive matmul from scratch is non-trivial; BLAS handles it correctly for all matrix layouts including transposed operands.

**Why OpenMP + simd?** Elementwise ops like `relu`, `sigmoid`, and `gelu` are embarrassingly parallel. `#pragma omp parallel for simd simdlen(4)` distributes work across CPU cores while the `simd` clause hints to the compiler to use AVX2 SIMD lanes (4 doubles per register with `-mavx2`). Personally, I felt making a project run using parallelism felt cool for some reason. Imagine telling some, I have a developed a parallelised calculation system, it fells euphoric, if you don't feel how I do, fuck you.
